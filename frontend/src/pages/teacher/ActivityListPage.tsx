...
const ActivityListPage: React.FC = () => {
  ...
  const loadActivities = async () => {
    try {
      setLoading(true);
      // Teachers can only see practice activities
      const response = await activityApi.getMyActivities({ ...filters, type: 'practice' });
  ...
  const handlePublish = async (activityId: number, currentStatus: string) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'status',
      width: 100,
      render: (_: any, record: Activity) => getStatusTag(record.status),
    },
    {
      title: '参与人数',
