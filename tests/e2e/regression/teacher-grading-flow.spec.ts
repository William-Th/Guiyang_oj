...
test.describe('Regression Tests - Teacher Grading Flow 教师评卷流程', () => {
  ...
  const navigateToGrading = async (page: Page) => {
    ...
    await page.waitForURL(/\/teacher\/grading/, { timeout: TEST_TIMEOUTS.NAVIGATION, waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle');
  ...
  test('GRD204 - 教师可以为题目评分', async ({ page }) => {
    ...
    await page.waitForURL(/\/teacher\/grading\/\d+/, { waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1000);
