...
test.describe('Regression Tests - 题库草稿箱与审核流程', () => {
  ...
  const createDraftQuestion = async (page: Page, questionData: {
    ...
    // 找到包含指定内容的行
    const targetRow = page.locator('.ant-table-tbody tr:visible').filter({ hasText: questionData.content }).first();
    await expect(targetRow).toBeAttached({ timeout: 5000 });

    // 提取题目编码（第二列为纯文本编码，如 MATH-0047）
    const codeCell = targetRow.locator('td').nth(1);
    const questionCode = await codeCell.textContent();
  ...
  test('R402 - 编辑草稿题目', async ({ page }) => {
    ...
    // 使用题目编码精确查找行
    const targetRow = page.locator('.ant-table-tbody tr:visible').filter({ hasText: questionCode }).first();
    await expect(targetRow).toBeAttached({ timeout: 5000 });
  ...
  test('R403 - 提交草稿进行审核', async ({ page }) => {
    ...
    // 选择目标题库范围 - 区级练习题库（审核人列表依赖所选范围）
    const scopeSelect = page.locator('.ant-modal .ant-select').first();
    await scopeSelect.click();
    await page.locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-item-option:has-text("区级练习题库")').first().click();
    await page.waitForTimeout(800); // 等待审核人列表按范围加载

    // 选择审核人（选择第一个可用的审核人）
    const reviewerSelect = page.locator('.ant-modal .ant-select').nth(1);
    await reviewerSelect.click();
    const firstReviewer = page.locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-item-option').first();
    await expect(firstReviewer).toBeVisible({ timeout: 5000 });
  ...
  test('R405 - 审核题目 - 批准通过', async ({ page }) => {
    ...
    // ========== 第一步：teacher_yy_ps_math 登录并创建草稿 ==========
    ...
    // ========== 第二步：teacher_yy_ps_math 提交审核并指定 teacher02 ==========
    console.log('Step 3: teacher_yy_ps_math 提交审核，指定 teacher02');
    ...
    const targetRow = page.locator('.ant-table-tbody tr:visible').filter({ hasText: `【R405-${timestamp}】` }).first();
    await expect(targetRow).toBeAttached({ timeout: 5000 });
    ...
    // 选择审核人 - 曹斌 (teacher_yy_ms_math)
    const reviewerSelect = page.locator('.ant-modal .ant-select').nth(1);
    await reviewerSelect.click();
    const reviewerOption = page.locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-item-option:has-text("曹斌")');
    await expect(reviewerOption).toBeVisible({ timeout: 5000 });
    console.log('选择审核人: 曹斌 (teacher_yy_ms_math)');
    ...
    const reviewTableRow = page.locator('.ant-table-tbody tr:visible').filter({
  ...
  test('R406 - 审核题目 - 拒绝', async ({ page }) => {
    ...
    const targetRow406 = page.locator('.ant-table-tbody tr:visible').filter({ hasText: `【R406-${timestamp}】` }).first();
    ...
    // 选择目标题库范围 - 区级练习题库（审核人列表依赖所选范围）
    const scopeSelect406 = page.locator('.ant-modal .ant-select').first();
    await scopeSelect406.click();
    await page.locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-item-option:has-text("区级练习题库")').first().click();
    await page.waitForTimeout(800); // 等待审核人列表按范围加载

    // 选择审核人 - 曹斌 (teacher_yy_ms_math)
    const reviewerSelect406 = page.locator('.ant-modal .ant-select').nth(1);
    await reviewerSelect406.click();
    const reviewerOption406 = page.locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-item-option:has-text("曹斌")');
    await expect(reviewerOption406).toBeVisible({ timeout: 5000 });
    console.log('选择审核人: 曹斌 (teacher_yy_ms_math)');
    await reviewerOption406.click();
    await page.waitForTimeout(500);
    ...
    const reviewTableRow406 = page.locator('.ant-table-tbody tr:visible').filter({
  ...
  test('R407 - 查看审核历史', async ({ page }) => {
    ...
    if (rowCount > 0) {
      // 点击查看历史按钮（使用更灵活的选择器）
      const historyButton = page.locator('.ant-table-tbody tr:visible').first().locator('button').filter({ hasText: '查看历史' }).or(page.locator('.ant-table-tbody tr:visible').first().locator('button').filter({ hasText: '历史' })).first();
  ...
  test('R409 - 多选适用范围提交审核', async ({ page }) => {
    ...
    const targetRow = page.locator('.ant-table-tbody tr:visible').filter({ hasText: `【R409-${timestamp}】` }).first();
    await expect(targetRow).toBeAttached({ timeout: 5000 });
    console.log('找到目标题目行');
    ...
    // 现行 UI 为单选范围：选择 测评题库（审核人列表依赖所选范围）
    console.log('选择题库范围：测评题库');
    const scopeSelect409 = page.locator('.ant-modal .ant-select').first();
    await scopeSelect409.click();
    await page.locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-item-option:has-text("测评题库")').first().click();
    await page.waitForTimeout(800); // 等待审核人列表按范围加载
    // 选择审核人 - 曹斌 (teacher_yy_ms_math)
    const reviewerSelect409 = page.locator('.ant-modal .ant-select').nth(1);
    await reviewerSelect409.click();
    const reviewerOption409 = page.locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-item-option:has-text("曹斌")');
    await expect(reviewerOption409).toBeVisible({ timeout: 5000 });
    console.log('选择审核人: 曹斌 (teacher_yy_ms_math)');
    await reviewerOption409.click();
    await page.waitForTimeout(500);
    ...
    console.log('teacher_yy_ps_math 多选范围提交审核成功');
  ...
  test('R410 - 被拒绝题目重新编辑并提交', async ({ page }) => {
    ...
    // 查找状态为"已拒绝"的行
    const rejectedRow = page.locator('.ant-table-tbody tr:visible').filter({
  ...
  test('QBK201 - Bug #7: 验证题库列表分页显示总数', async ({ page }) => {
    ...
    await activeTab.locator('input[placeholder="用户名"]').fill('teacher_by_ps_math');
  ...
  test('QBK203 - Bug #9: 验证搜索题目内容和编码功能', async ({ page }) => {
    ...
    // Step 2: 验证搜索区域存在
    const searchInput = page.locator('input[placeholder*="题目内容"]').first();
