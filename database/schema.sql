...
BEGIN
  ...
  CASE p_subject
    ...
    WHEN '计算�? THEN v_subject_code := 'COMP';
...
COMMENT ON FUNCTION public.log_registration_action(p_request_id integer, p_action character varying, p_action_by integer, p_action_level integer, p_comment text, p_metadata jsonb) IS '记录注册审核操作日志的辅助函�?;
...
COMMENT ON FUNCTION public.update_student_activity_grading_status() IS '自动更新学生活动的评卷状�?;
...
BEGIN
  ...
  IF NEW.time_limit_type = 'unlimited' THEN
    IF NEW.start_time IS NOT NULL OR NEW.end_time IS NOT NULL OR NEW.duration IS NOT NULL THEN
      RAISE EXCEPTION '无时间限制类型不应设置时间字�?;
    END IF;
  END IF;
  ...
  IF NEW.time_limit_type = 'scheduled' THEN
    ...
    IF NEW.duration IS NOT NULL THEN
      RAISE EXCEPTION '固定时间段类型不应设置作答时�?;
    END IF;
    IF NEW.end_time <= NEW.start_time THEN
      RAISE EXCEPTION '结束时间必须晚于开始时�?;
    END IF;
  END IF;
  ...
  IF NEW.time_limit_type = 'timed' THEN
    ...
    IF NEW.start_time IS NOT NULL OR NEW.end_time IS NOT NULL THEN
      RAISE EXCEPTION '限时作答类型不应设置固定时间�?;
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.validate_teacher_permission() IS '验证教师权限�?scope_level �?district_id/school_id 匹配�?;
...
COMMENT ON TABLE public.achievement_progress IS '成就进度跟踪�?;
...
COMMENT ON COLUMN public.achievements.rarity IS '稀有度：common(普�?/rare(稀�?/epic(史诗)/legendary(传说)/mythic(神话)';
...
COMMENT ON COLUMN public.achievements.trigger_condition IS '触发条件JSON，包含trigger_mode, trigger_frequency, condition_type�?;
...
COMMENT ON COLUMN public.activities.total_score IS '试卷总分 (所有题目分值之�?';
...
COMMENT ON COLUMN public.activities.paper_status IS '组卷状�? empty=未组�? draft=草稿�? completed=已完�?;
...
COMMENT ON COLUMN public.student_activities.started_at IS '开始作答时�?(用于timed类型计算deadline)';
...
COMMENT ON COLUMN public.student_activities.grading_status IS '评卷状�? pending-待评�? auto_graded-自动评卷完成, partial_graded-部分评卷, completed-全部完成';
...
COMMENT ON TABLE public.activity_questions IS '活动题目关联�?- 存储活动（练�?测评）与题目的关联关系，支持组卷功能';
...
COMMENT ON COLUMN public.activity_questions.activity_id IS '关联的活动ID (activities表外�?';
...
COMMENT ON COLUMN public.activity_questions.question_id IS '关联的题目ID (question_bank表外�?';
...
COMMENT ON COLUMN public.activity_questions.order_index IS '题目在试卷中的显示顺�?(1,2,3...)';
...
COMMENT ON COLUMN public.activity_questions.score IS '该题目在本活动中的分�?(可与题库中建议分值不�?';


--
-- Name: COLUMN activity_questions.is_required; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.activity_questions.is_required IS '是否必答�?(true=必答, false=选答)';


--
-- Name: COLUMN activity_questions.section; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.activity_questions.section IS '题目所属部分或章节 (例如：第一部分、单选题、多选题�?';
...
COMMENT ON COLUMN public.question_bank.suggested_score IS '建议分�?;
...
COMMENT ON COLUMN public.question_bank.status IS '题目状态：draft草稿，pending_review待审核，approved已批准，rejected已拒绝，published已发�?;
...
COMMENT ON COLUMN public.answers.grading_status IS '该题评卷状�? pending-待评�? auto_graded-自动评卷, manual_graded-人工评卷';
...
COMMENT ON COLUMN public.answers.feedback IS '评卷批注和反�?;
...
COMMENT ON TABLE public.daily_tasks IS '日常任务定义�?;
...
COMMENT ON COLUMN public.daily_tasks.trigger_condition IS 'JSON格式的触发条�?;
...
COMMENT ON COLUMN public.daily_tasks.target_value IS '任务目标数�?;
...
COMMENT ON COLUMN public.daily_tasks.reset_time IS '每日重置时间�?;
...
COMMENT ON COLUMN public.daily_tasks.valid_from IS '任务有效期起始日�?;
...
COMMENT ON COLUMN public.daily_tasks.valid_to IS '任务有效期结束日�?;
...
COMMENT ON TABLE public.districts IS '区域管理表，支持区级和市级管�?;
...
COMMENT ON TABLE public.points_transactions IS '积分交易明细�?;
...
COMMENT ON VIEW public.question_bank_distribution IS '题库分布统计：按 scope、科目、年级统计题目数�?;
...
COMMENT ON TABLE public.registration_audit_log IS '注册审核日志表，记录所有审核操作历�?;
...
COMMENT ON COLUMN public.registration_audit_log.action_level IS '操作层级: 0=系统, 2=校级, 3=区县�? 4=市级';
...
COMMENT ON COLUMN public.registration_audit_log.metadata IS 'JSON格式元数据，存储升级原因等额外信�?;
...
COMMENT ON TABLE public.student_achievements IS '学生成就记录�?;
...
COMMENT ON TABLE public.student_login_history IS '学生登录历史记录表，用于成就系统和行为分�?;
...
COMMENT ON COLUMN public.student_login_history.user_agent IS '用户代理字符�?;
...
COMMENT ON TABLE public.student_daily_tasks IS '学生日常任务完成记录�?;
...
COMMENT ON COLUMN public.student_daily_tasks.task_date IS '任务日期，每日凌晨重�?;
...
COMMENT ON TABLE public.student_points IS '学生积分账户�?;
...
COMMENT ON COLUMN public.student_points.current_points IS '当前可用积分 = 总积�?- 已消�?- 冻结';
...
COMMENT ON COLUMN public.student_registration_requests.current_reviewer_level IS '当前审核层级: 2=校级, 3=区县�? 4=市级';
...
COMMENT ON COLUMN public.student_registration_requests.last_escalated_at IS '最后升级时间，用于计算3天自动升�?;
...
COMMENT ON TABLE public.student_task_progress IS '学生日常任务进度�?;
...
COMMENT ON COLUMN public.student_task_progress.current_value IS '当前完成进度�?;
...
COMMENT ON COLUMN public.student_task_progress.bonus_awarded IS '获得的奖励积�?;
...
COMMENT ON COLUMN public.student_task_progress.period_start IS '任务周期开始日�?;
...
COMMENT ON COLUMN public.student_task_progress.reset_count IS '任务重置次数，用于连续完成追�?;
...
COMMENT ON TABLE public.task_completion_history IS '任务完成历史记录�?;
...
COMMENT ON COLUMN public.task_completion_history.bonus_earned IS '获得的奖励积�?;
...
COMMENT ON VIEW public.teacher_grading_workload IS '教师评卷工作量统计视�?;
...
--
-- Name: idx_activity_questions_activity_section; Type: INDEX; Schema: public; Owner: -
--
...
COMMENT ON TABLE student_ability_stats IS '学生能力统计�?- 记录每个学生在各能力维度上的表现';
...
COMMENT ON COLUMN student_ability_stats.total_questions IS '总题�?;
COMMENT ON COLUMN student_ability_stats.correct_count IS '正确题数';
COMMENT ON COLUMN student_ability_stats.accuracy_rate IS '正确�?%)';
...
COMMENT ON TABLE student_knowledge_stats IS '学生知识点统计表 - 记录每个学生在各知识点上的掌握情�?;
COMMENT ON COLUMN student_knowledge_stats.student_id IS '学生用户ID';
COMMENT ON COLUMN student_knowledge_stats.knowledge_point IS '知识点标�?;
COMMENT ON COLUMN student_knowledge_stats.subject IS '科目';
COMMENT ON COLUMN student_knowledge_stats.total_questions IS '总题�?;
COMMENT ON COLUMN student_knowledge_stats.correct_count IS '正确题数';
COMMENT ON COLUMN student_knowledge_stats.accuracy_rate IS '正确�?%)';
...
COMMENT ON TABLE school_ability_stats IS '学校能力统计�?- 记录学校在各能力维度上的整体表现';
...
COMMENT ON COLUMN school_ability_stats.student_count IS '参与学生�?;
COMMENT ON COLUMN school_ability_stats.total_attempts IS '总答题次�?;
COMMENT ON COLUMN school_ability_stats.correct_count IS '正确次数';
COMMENT ON COLUMN school_ability_stats.accuracy_rate IS '正确�?%)';
COMMENT ON COLUMN school_ability_stats.avg_score IS '平均得分';
COMMENT ON COLUMN school_ability_stats.period_start IS '统计周期开始日�?;
...
COMMENT ON TABLE district_ability_stats IS '区域能力统计�?- 记录区域在各能力维度上的整体表现';
...
COMMENT ON COLUMN district_ability_stats.school_count IS '参与学校�?;
COMMENT ON COLUMN district_ability_stats.student_count IS '参与学生�?;
COMMENT ON COLUMN district_ability_stats.total_attempts IS '总答题次�?;
COMMENT ON COLUMN district_ability_stats.correct_count IS '正确次数';
COMMENT ON COLUMN district_ability_stats.accuracy_rate IS '正确�?%)';
COMMENT ON COLUMN district_ability_stats.avg_score IS '平均得分';
COMMENT ON COLUMN district_ability_stats.period_start IS '统计周期开始日�?;
...
COMMENT ON VIEW v_student_ability_realtime IS '学生能力实时统计视图 - 基于最新答题记录实时计�?;
...
COMMENT ON VIEW v_student_knowledge_realtime IS '学生知识点实时统计视�?- 基于最新答题记录实时计�?;
...
COMMENT ON VIEW v_school_ability_realtime IS '学校能力实时统计视图 - 基于学校学生的答题记�?;
...
COMMENT ON FUNCTION update_student_ability_stats() IS '触发器函数：学生答题后自动更新能力统�?;
