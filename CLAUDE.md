...
3. **修复后必须验证**
   - 修改代码后必须重新构建Docker服务
   - **必须手动测试验证**修复是否生效
   - 验证通过后才能提交代码和更新状态为"待复核"
   - 如果验证失败，继续调试，状态保持"进行中"
...
7. **🔴 未测试的代码不应提交**
   - **Bug修复必须先测试验证**: Bug修复代码必须经过手动测试验证通过后才能提交
   - **新功能必须有测试**: 新功能代码必须附带API测试或E2E测试，测试通过后才能提交
   - **代码变更必须验证**: 任何代码变更都必须经过验证（手动测试或自动化测试），确保功能正常后再提交
   - **禁止未验证提交**: 严禁提交未经测试验证的代码，即使是"看起来正确"的代码也必须测试

   **验证要求**:
   - Bug修复: 手动测试验证问题已解决 → 更新BUG_FIX_TRACKING.csv状态为"待复核" → 用户复核通过后标记"已解决"
   - 新功能/API变更: 编写并运行API测试，确保测试通过
   - 前端UI变更: 编写并运行E2E测试，确保测试通过
   - 重构/优化: 确保现有测试仍然通过，必要时添加新测试

   **提交前检查清单**:
   - [ ] 代码已通过手动测试或自动化测试验证
   - [ ] Bug修复已在BUG_FIX_TRACKING.csv中记录状态
   - [ ] 新功能已添加相应的测试用例
   - [ ] 所有相关测试都已通过
   - [ ] Docker服务已重新构建并验证功能正常
