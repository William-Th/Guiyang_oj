...
### 2026-09-20（补：用户管理市级管理员筛选不一致）
- 🐛 **统计卡与筛选口径冲突**：用户管理「市级管理员」卡片把 municipal_admin + municipal_school_admin 合并计数显示 1，但筛选「市级总管理员」永远无数据——后端 /users/all 对市级总管理员视角会排除 system_admin/municipal_admin（唯一 municipal_admin 是其自身），合并计数里的 1 实为市直属学校管理员。修复：卡片拆分为「市级总管理员」「市直属学校管理员」各自独立计数；市级管理员视角的筛选下拉移除永远为空的市级总管理员选项（系统管理员视角不受影响）

### 2026-09-20（补：全局主题换色 + 表格紧凑化）
- 🎨 **主题色整体切换为天空蓝**：主色 #0ea5e9；信息色跟随主题；成功 #52c41a；警告 #faad14；错误 #f5222d。boheTheme.ts（antd token）与 variables.css（CSS 变量）全量重写色板，并以映射脚本统一替换 45 个文件中散落的旧薄荷绿/青色硬编码（#4D9899/#7AC99C/#16a34a 系 → sky 色阶；#fa8c16→#faad14；#ff4d4f→#f5222d）。头部改纯色 #0284c7（白字对比达标），顶部导航去除用户信息胶囊、菜单项紧凑居左、图标-文字间距收紧至 3px
- 🧮 **表格紧凑化（参考 Element Plus / Naive UI）**：Table 组件 token——表头浅灰底 #f7f8fa + 中性灰字、行悬停淡主色 #f0f9ff、分隔线 #f1f5f9、单元格内边距 16px→9/12、字号 13.5；并加全局 CSS 兜底（表头 nowrap、细边框）。解决「间距过大、看着一大块」的问题
...
### 2026-02-27
- 🐛 **修复学生答题页面分数显示错误**
  - 问题：题型总分显示错误（如"共010.0010.0010.0010.00分"）
  - 原因：API返回的`max_score`是字符串类型，直接相加变成字符串拼接
  - 解决：使用`parseFloat()`转换为数字后再相加
  - 文件：`frontend/src/pages/student/TakeActivityPage.tsx`
  - 状态: ✅ 已完成
- 🎨 **学生答题页面按题型分组显示**
  - 问题：题目序号连续显示，与试卷预览格式不一致
  - 解决：按题型分组渲染，每题型独立编号
  - 功能：
    1. 题型分组标题：一、单选题（共X题，共X分）
    2. 每题型内部从1开始编号
    3. 答题卡按钮显示题型内序号
    4. 添加"匹配题"类型支持
  - 文件：`frontend/src/pages/student/TakeActivityPage.tsx`
...
- 🐛 **修复组卷页面题型显示问题**
  ...
  - 状态: ✅ 已完成
- 🎨 **组卷页面改为横向Tabs布局**
  - 问题：折叠面板(Collapse)布局是垂直的，需要上下翻动查看不同题型
  - 解决：改为水平Tabs布局，题型标签横向排列在顶部
  - 功能：
    1. 顶部横向Tabs：全部、单选题、多选题、判断题、填空题等
    2. 每个Tab显示题目数量和已选数量
    3. 点击Tab切换显示该题型的题目列表
    4. 保留横向卡片布局（每行2题）
    5. 保留全选/批量添加/分页功能
  ...
  - 状态: ✅ 已完成
- 🎨 **优化组卷页面题型选择体验**
  - 改为横向卡片布局，每行2题，避免上下翻动
  - 添加题型分页功能（每页10题），手动上下页切换
  - 卡片显示：题号、难度、题干预览、分值、级别、已选状态
  - 点击卡片切换选中状态，点击按钮直接添加
  - 题型中文名称映射（判断题等）
  - 文件：`frontend/src/pages/teacher/PaperGenerationPage.tsx`
  - 状态: ✅ 已完成
- 🎨 **组卷页面按题型分类显示**
  - 问题：题目列表杂乱，无法按题型区分选择
  - 解决：使用折叠面板(Collapse)按题型分组显示题目
  - 功能：
    1. 单选题、多选题、判断题、填空题、问答题、编程题分类显示
    2. 每个题型显示题目数量和已选数量
    3. 每个题型面板独立的全选/取消全选/批量添加按钮
  - 文件：`frontend/src/pages/teacher/PaperGenerationPage.tsx`
  - 状态: ✅ 已完成
- 🐛 **修复练习中心时长显示null问题**
  - 问题：练习时长显示 `null分钟`
