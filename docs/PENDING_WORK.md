...
**最后更新**: 2026-05-14（设计系统重构完成 + 文档状态核对）

---

## 📌 顶层状态快照（2026-05-14）

本节是**最新事实**。下方"🔥 当前工作状态"和后续章节属于历史记录，可能与此处冲突，
若冲突以本节为准。

### ✅ 近期已完成（在本分支 feature/achievement-system 上）

| 项目 | 状态 | 关键 commit |
|------|------|------------|
| 题库管理权限重命名 (_review → _manage) + 撤回功能 | ✅ 完成 | e52528e |
| 评卷列表丢失自动判题活动 bug | ✅ 修复 | 50ad874 (autoGradingService.js) |
| 判断题独立判题（不再归类到 single） | ✅ 完成 | 50ad874 + 单测 18 个 |
| 匹配题自动判题（按比例给分） | ✅ 完成 | 551070d + 单测 13 个 |
| 学生端结果按题型分组显示 | ✅ 完成 | 551070d (ActivityResultPage) |
| 教师端批改界面按题型分组（含侧边栏导航） | ✅ 完成 | 551070d (GradingDetailPage) |
| 登录守卫加固（同步跳转，消除首页闪现） | ✅ 完成 | 50ad874 (MainLayout) |
| 测评报名功能（locations + registrations 全套） | ✅ 已在更早 commit 49e4dfd 完成 | 文档误标待更新 |
| 薄荷设计系统接入（A: 单点主题源） | ✅ 完成 | 69b32b8 |
| 薄荷设计系统重构（B+C+D: 84+ 处 hex 替换） | ✅ 完成 | 6620fe3 |
| BUG-001/002/004（首页未登录跳转、草稿箱按钮、校级管理员菜单） | ✅ 早已修复 | ce30970（PENDING_WORK 旧条目未更新） |
| BUG-005（区级题目无可选审核人） | ✅ 已修 | bb9ebee（PENDING_WORK 早已标注） |
| 主分支同步 | ✅ main 已 fast-forward 到 feature/achievement-system | 含本分支全部成果 |

### 🟡 仍未完成（高 ROI 优先）

| 优先级 | 项目 | 说明 |
|--------|------|------|
| P1 | GRD001-004 E2E 测试 | 判断题/匹配题判题 + 题型分组显示的端到端验证 |
| P1 | E2E 测试缺口补齐 | JWT 刷新/登出/密码修改、删除题目、题目搜索、用户管理（学生/教师/批量导入）|
| P1 | 题库审核流 E2E (R403-R406) | 提交审核/审核题目/发布题目/驳回题目 |
| P1 | BUG-003 题目保存逻辑重设计 | 新建时不选范围，发布时再选 |
| P2 | Frontend Refactoring Phase 3 (Batch 2-5) + Phase 4-5 | ~158 个 lint warning，max-warnings 当前临时调至 420 |
| P2 | 编程题判题：测评中端到端集成测试、教师创建编程题表单完善 |  |
| P2 | 内存限制真实检测 / 特判程序 / Monaco 代码补全 |  |
| P2 | 通知系统 P3 扩展（邮件通道、偏好设置） |  |

### 🟦 暂停 / 大块未启动

| 项目 | 工期 | 说明 |
|------|------|------|
| Activity 系统精细化 | ~1-1.5 周 | 教师练习管理只显练习、管理员测评管理独立 API、学生测评中心详情完善 |
| 练习时间限制类型 (PTL) | ~1 周 | 后端 API 已完成；前端倒计时组件 + cron 自动提交未做 |
| 成就系统 Week 4-14 | ~9 周 | 日常任务已部分实现；排行榜、社交、通知提醒等未启动 |

### ⚠️ 文档冲突已统一修正

- `docs/DEVELOPMENT_STATUS.md` 7.4 节"测评报名管理系统"全部状态从 ❌ 修正为 ✅（commit 551070d）
- 本文件 BUG-001/002/004 条目（line ~806-961）显示为"待修复"，实际早在 ce30970 已修复
...
| Bug编号 | 问题描述 | 状态 | 完成日期 |
|---------|---------|------|----------|
| Bug #18 | 题库管理中题库范围筛选选项不全 | ✅ 已修复 | 2025-11-22 |
| Bug #19 | 活动管理中时长显示为null分钟 | ✅ 已修复 | 2025-11-22 |
| Bug #20 | 练习活动缺少组卷按钮 | ✅ 已修复 | 2025-11-22 |
| Bug #21 | 菜单名称不统一显示为活动管理 | ✅ 已修复 | 2025-11-22 |
| Bug #22 | 区级题目发布应自动匹配账号所属区域 | ✅ 已修复 | 2025-11-22 |
| **Bug #23** | **校级管理员获取不到区域信息** | ✅ **已修复（代码+数据）** | **2025-11-22** |

**Bug #23 修复详情**:
- **问题**: 43个school_admin用户的admin_permissions.district_id为NULL
- **代码修复**: 在User.getDetailedProfile()中添加fallback逻辑
- **数据修复**: 创建迁移脚本，批量更新43条记录
- **影响范围**: 所有校级管理员现在都能正确获取区域信息并发布区级题目
- **提交记录**:
  - Commit c22cdec: 代码修复
  - Commit 08eb27d: 数据迁移

**相关文件**:
- `backend/src/models/User.js` - 添加fallback逻辑
- `database/migrations/fix-admin-permissions-district-id.sql` - 数据迁移脚本
- `docs/BUG_FIX_TRACKING.csv` - Bug #18-23追踪记录

**下一步**: 用户复核验证所有Bug修复是否正确
